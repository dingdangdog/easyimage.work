<template>
  <div class="bg-gray-50 dark:bg-gray-900 h-screen flex items-center justify-center py-20 px-4">
    <div
    ...
    >
      <div
        class="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300"
      >
        <span class="text-2xl font-bold">{{ displayCode }}</span>
      </div>

      <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-semibold mb-3">
        {{ title }}
      </h1>
...
const is404 = computed(() => {
  const status =
    (error.value as any)?.statusCode || (error.value as any)?.status;
  return Number(status) === 404;
});

const displayCode = computed(() => {
  const status =
    (error.value as any)?.statusCode || (error.value as any)?.status || 500;
  return status;
});

const title = computed(() => {
  return is404.value
    ? t("base.page_not_found", "Page Not Found")
    : t("base.something_went_wrong", "Something went wrong");
});

const description = computed(() => {
  return is404.value
    ? t(
        "base.page_not_found_desc",
        "The page you are looking for does not exist or has been moved."
      )
    : t(
        "base.error_generic_desc",

      ...
      <div class="space-y-6">
        ...
        >
          ...
          <div class="flex flex-col lg:flex-row gap-4">
            ...
            <!-- AI信息显示 -->
            <div class="bg-gray-100 dark:bg-gray-700 rounded-lg p-2 w-full relative">
              <div v-if="result.aiInfo && result.aiInfo.rawText">
                <!-- 标题和复制按钮 -->
                <button
                  @click="copyToClipboard(result.aiInfo.rawText)"
                  class="absolute top-2 right-2 px-2 py-1 bg-blue-500/20 hover:bg-blue-500/80 text-white text-sm font-medium rounded-lg transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300"
                >
                  {{ $t("aiAnalysis.copy") }}
                </button>
                <!-- 完整信息显示 -->
...
const formatAIText = (rawText: string): string => {
  ...
  if (paramMatch && paramMatch[1]) {
    ...
    // 处理Prompt部分
    if (sections[0]) {
      const promptText = sections[0]
        .replace(/\n+/g, " ")
        .trim();
      
      if (promptText && !promptText.toLowerCase().startsWith("negative prompt")) {
        formattedText += `Prompt:\n${promptText}\n\n`;
      }
    }
    
    // 处理Negative prompt部分
    const negativeSection = sections.find(s => s.trim().startsWith("Negative prompt:"));
    if (negativeSection) {
      const negativeMatch = negativeSection.match(/Negative prompt:\s*(.*?)(?=\n*Steps:|$)/s);
    ...
    // 查找negative prompt
    const negativeMatch = rawText.match(/Negative prompt:\s*(.*?)(?=\n(?:Steps:|$))/s);
    if (negativeMatch) {
